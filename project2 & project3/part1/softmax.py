...
def compute_probabilities(X, theta, temp_parameter):
    ...
    # n = X.shape[0]
    # k = theta.shape[0]
    # H = np.empty(shape=(n,k), dtype="object")
    # for i in range(n):
    #     power = theta.dot(X[i])/temp_parameter
    #     c = np.amax(power)
    #     numerator = np.exp(power - c)
    #     denominator = np.sum([numerator])
    #     H[i] = numerator/denominator
    # return np.transpose(H)
    power = theta.dot(X.T)/temp_parameter
    c = np.amax(power, axis=0)
    H = (np.exp(power - c))/np.sum(np.exp(power-c), axis=0)
    return H
